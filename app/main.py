...
DEFAULT_RATE_LIMIT = int(os.getenv("DEFAULT_RATE_LIMIT", "5"))
...
async def rate_limit_middleware(request: Request, call_next):
    # Bypass health check and openapi routes
    ...
    if is_limited:
        return JSONResponse(
            ...
            content={
                ...
                "message": f"Tenant '{tenant_id}' exceeded quota limit of {DEFAULT_RATE_LIMIT} requests per minute.",
    ...
    response.headers["X-RateLimit-Remaining"] = str(max(0, DEFAULT_RATE_LIMIT - current_usage))
